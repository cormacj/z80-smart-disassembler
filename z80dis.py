...
def handle_data(b): #, loc, code_org):
    if (b.operands[0][0] is b.operands[0][0].REG) and (b.operands[1][0] is b.operands[0][0].REG):
        return None
...
def handle_jump(b, loc, code_org):
    ...
    if b.operands[0][0] is not b.operands[0][0].REG_DEREF: #if not a JP (HL)
...
print(";Pass 0: Prep")
identified_areas = {code_org + loc: "" for loc in range(len(bin_data) + 10)}
...
while loc < len(bin_data):
    ...
    if identified_areas[code_org + loc] == "D" and (loc + code_org) in str_locations:
        ...
        if b.op in (b.op.JR, b.op.DJNZ):
            jump_addr = handle_jump(b, loc, code_org)
            if jump_addr:
                tmp = f"{b.op.name} " + lookup_label(jump_addr)
